...
// Técnica de Grounding 5-4-3-2-1 - VERSIÓN FINAL FUNCIONAL Y BONITA
...
export class GroundingGame {
            ...
            <div class="grounding-game">
                ...
                <div class="current-step-container">
                    ...
                    <p class="step-description">
                        ${this.getDescription(step.id, remaining)}
                    </p>
                    
                    <div class="items-list">
                        ${step.items.map(item => `
                            <div class="item-pill">
                                <span>✓</span> ${this.escapeHtml(item)}
                            </div>
                        `).join('')}
                    </div>
                    ...
                    ${step.items.length < step.number ? `
                        <div class="input-group">
                            <input 
                                ...
                                placeholder="${this.getPlaceholder(step.id)}"
                                autocomplete="off"
                            >
                    ...
                    ` : `
                        <div class="step-completed-message">
                            <span>✨</span> ¡Completado! <span>✨</span>
                        </div>
                        ${this.currentStepIndex < this.steps.length - 1 ? `
                            <button id="nextStepBtn" class="btn-primary" style="margin-top: 16px; width: 100%;">
                                Siguiente paso →
                            </button>
                        ` : `
                            <button id="finishGameBtn" class="btn-primary" style="margin-top: 16px; width: 100%; background: linear-gradient(135deg, #10b981, #059669);">
                                🌟 Finalizar grounding
                            </button>
                        `}
                    `}
                ...
                <div style="display: flex; gap: 16px; justify-content: center; margin-top: 32px;">
                    <button id="backFromGrounding" class="btn-secondary">
                        ← Volver a juegos
                    </button>
                </div>
            </div>
    ...
    addItem(value) {
        ...
        if (step.items.includes(value)) {
            this.showToast('✨ Ya anotaste eso, intenta con otra cosa', 'info');
            return;
        }
    ...
    finishGame() {
        ...
        if (!allCompleted) {
            this.showToast('⚠️ Aún hay pasos sin completar', 'info');
            return;
        }
                ...
                <div class="completion-summary">
                    ${this.steps.map(step => `
                        <div class="summary-step">
                            <span>${step.icon}</span>
                            <span>${step.number} ${step.label.toLowerCase()}</span>
                            <span style="font-size: 0.8rem; color: #10b981;">✓</span>
                        </div>
                    `).join('')}
                </div>
                <div style="margin-top: 32px;">
                    <button id="backAfterComplete" class="btn-primary" style="background: linear-gradient(135deg, #10b981, #059669);">
                        ← Volver a juegos
                    </button>
                </div>
            </div>
        ...
        if (window.app && window.app.anxietyState) {
            ...
            toast.innerText = '🌍 ¡Grounding completado! Has vuelto al presente.';
        ...
        document.getElementById('backAfterComplete')?.addEventListener('click', () => {
            if (window.app && window.app.router) {
                window.app.router.showGamesView();
            }
        });
...
export function initGrounding(container) {
    const game = new GroundingGame(container);
    game.render();
    return game;
}