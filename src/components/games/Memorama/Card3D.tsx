...
export default function Card3D({ position, texture, cardState, shake, color, onClick }: Props) {
  ...
  useFrame((state, dt) => {
    ...
    if (bubbleRef.current) bubbleRef.current.scale.setScalar(Math.max(0,bScale.current));
  ...
  return (
    <group ref={gRef} position={position} onClick={onClick}
      onPointerEnter={()=>setHov(true)} onPointerLeave={()=>setHov(false)}>

      <mesh ref={bubbleRef}>
        <sphereGeometry args={[0.75, 16, 12]} />
        <meshBasicMaterial color="#88ccff" transparent opacity={0.12} depthWrite={false} />
      </mesh>
      ...
      {/* ─ Card back (question mark face) ─ */}
      <group position={[0,0,.012]}>
        <mesh>
          <planeGeometry args={[CW,CH]}/>
          <meshBasicMaterial color={hov?'#221a55':'#130f3a'}/>
        </mesh>
        <mesh position={[0,0,.001]} rotation={[0,0,Math.PI/4]}>
          <planeGeometry args={[.22,.22]}/>
          <meshBasicMaterial color={color} transparent opacity={.22}/>
        </mesh>
        {[-1,1].map((d,i)=>(
          <mesh key={i} position={[0,0,.001]} rotation={[0,0,d*Math.PI/4]}>
            <planeGeometry args={[.58,.014]}/>
            <meshBasicMaterial color={color} transparent opacity={.40}/>
          </mesh>
        ))}
        {([[-CW/2+.09,CH/2-.09],[CW/2-.09,CH/2-.09],
          ...
          <mesh key={i} position={[cx,cy,.002]}>
            ...
            <meshBasicMaterial color={color} transparent opacity={.70}/>
          </mesh>
        ))}
      </group>
      ...
      <group position={[0,0,-.012]} rotation={[0,Math.PI,0]}>
        ...
        <mesh position={[0,0,.001]}>
          ...
          <meshBasicMaterial
            ...
            transparent opacity={cardState==='matched'?.28:.14}/>
        </mesh>
      </group>
