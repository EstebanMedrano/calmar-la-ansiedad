...
interface FishCfg {
  ...
  sz: number; spd: number; ph: number;
}
...
function Fish({ cfg }: { cfg: FishCfg }) {
  ...
  useFrame((_, dt) => {
    ...
    const dx = -Math.sin(t.current) * cfg.rx;
    const dz = Math.cos(t.current) * cfg.rz;
  ...
  return (
    <group position={[2, -6.5, -12]} rotation={[0, .5, .18]}>
      <mesh position={[0, 0, 0]}>
        ...
        <meshStandardMaterial color="#3a2818" roughness={.95} />
      </mesh>
      <mesh position={[.5, 1.1, 0]}>
        <boxGeometry args={[2.5, 1.2, 1.8]} />
        <meshStandardMaterial color="#2a1c10" roughness={.95} />
      </mesh>
      <mesh position={[0, 2.8, 0]}>
        <cylinderGeometry args={[.09, .09, 3.5, 6]} />
...
// ── Fog & Background Transition ────────────────────────────────────────────
function FogTransition({ introStage }: { introStage: IntroStage }) {
  const { scene } = useThree();
  const fogColor = useRef(new THREE.Color('#7ec8e3'));
  const bgColor = useRef(new THREE.Color('#7ec8e3'));
  const fogNear = useRef(30);
  const fogFar = useRef(90);

  useEffect(() => {
    scene.fog = new THREE.Fog(fogColor.current, fogNear.current, fogFar.current);
    scene.background = new THREE.Color('#7ec8e3');
    return () => { scene.fog = null; scene.background = null; };
  }, [scene]);

  useFrame(() => {
    let targetColor: string, targetNear: number, targetFar: number;
    switch (introStage) {
      case 'beach':
        targetColor = '#7ec8e3'; targetNear = 30; targetFar = 90; break;
      case 'surface':
        targetColor = '#5a9ab8'; targetNear = 25; targetFar = 75; break;
      case 'diving':
        targetColor = '#1a3a5c'; targetNear = 15; targetFar = 55; break;
      case 'playing':
      default:
        targetColor = '#0a1a3c'; targetNear = 10; targetFar = 45; break;
    }
    const tc = new THREE.Color(targetColor);
    fogColor.current.lerp(tc, 0.03);
    bgColor.current.lerp(tc, 0.03);
    fogNear.current += (targetNear - fogNear.current) * 0.03;
    fogFar.current += (targetFar - fogFar.current) * 0.03;
    if (scene.fog) {
      (scene.fog as THREE.Fog).color.copy(fogColor.current);
      (scene.fog as THREE.Fog).near = fogNear.current;
      (scene.fog as THREE.Fog).far = fogFar.current;
    }
...
export default function OceanEnvironment({ introStage }: { introStage: IntroStage }) {
  ...
  return (
    <>
      <FogTransition introStage={introStage} />

      <ambientLight color={isBeach ? "#d0e4ff" : "#2a5a8a"} intensity={isBeach ? 0.8 : 0.55} />
      {isUnderwater && (
        <hemisphereLight color="#4488ff" groundColor="#001a33" intensity={0.8} />
      )}
      ...
      {isUnderwater && (
        <>
          ...
          <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -6, 0]}>
            ...
            <meshStandardMaterial color="#3a2a1a" roughness={.95} />
          </mesh>

          {/* Pared de fondo */}
          <mesh position={[0, 0, -15]}>
            <planeGeometry args={[50, 30]} />
            <meshBasicMaterial color="#2e2797" transparent opacity={.88} />
          </mesh>

          {/* ⭐ LA ESFERA GIGANTE HA SIDO ELIMINADA. EL FONDO AHORA LO MANEJA `scene.background` */}
          ...
          {([
            ...
            [0, -6, -9, '#ff6688'],
            [-6, -6, -8, '#cc44ff'],
          ] as [number, number, number, string][]).map(([x, y, z, c], i) => (
            <Coral key={i} pos={[x, y, z]} color={c} />
          ))}
          {([
            [-4, -5.8, -6, 1.2],
            [-6, -5.5, -9, .9],
            ...
            <mesh key={i} position={[x, y, z]} scale={s}>
              ...
              <meshStandardMaterial color="#3a3428" roughness={.95} />
            </mesh>
          ))}
        </>
      )}
